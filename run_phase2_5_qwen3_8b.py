#!/usr/bin/env python3
"""
Phase 5 — Full Fine-Tuning: Qwen3-8B

Full parameter fine-tuning (no LoRA/QLoRA) of Qwen/Qwen3-8B.
Saves complete model weights to ./models/qwen3_8b_finetuned/

Run in background:
    nohup python run_phase5.py > logs/phase5_qwen3_8b.log 2>&1 &

Monitor:
    tail -f logs/phase5_qwen3_8b.log
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/phase5_qwen3_8b.log", mode="a"),
    ],
)
...
CONFIG = {
    ...
    "OUTPUT_DIR":            Path("./models/qwen3_8b_finetuned"),
    "DATA_DIR":              Path("."),
    ...
    "LEARNING_RATE":         2e-4,    # FIXED: 20× increase (was 1e-5, now same as QLoRA)
    "NUM_TRAIN_EPOCHS":      3,
...
def main():
    ...
    if hf_token:
        ...
        log.warning("HF_TOKEN not set — may fail if Qwen3-8B is gated")
    ...
    log.info("\n" + "="*65)
    log.info("  PHASE 5 — Full Fine-Tuning: Qwen3-8B")
